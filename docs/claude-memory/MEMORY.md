...
- [Monitoring/reporting + kill-switch 2026-07-01](project_monitoring_config_2026_07.md) — Routine perf alerts silenced (PERF_ROUTINE_ALERTS_ENABLED=False). Daily Review merged into Daily Summary (build_ops_health). Reports filtered to config.active_traded_symbols(). Kill switch kept DORMANT (demo/testing) with decoupled baseline PERF_KILL_SWITCH_SINCE=2026-07-01, count reset 179→0.
...
- [Order placement false-failure orphan](feedback_order_placement_false_failure.md) — place_market_order returned None on timeout/ambiguous retcode while the broker filled it → orphan (LONDON_BO 2026-06-29, +$85.56 untracked). Now self-recovers via before/after position diff (commit 1f934a4).
