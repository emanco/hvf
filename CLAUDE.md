...
- **Active strategies** (each its own scanner thread; `ENABLED_PATTERNS` for the main loop is now `[]`):
  ...
  - **ASIAN_SESSION_BREAKOUT (ASB)** — Asian-range breakout on GBPJPY + USDJPY + EURUSD (EURJPY dropped 2026-06-26; **USDJPY added 2026-07-16** via pre-registered pair screen `scripts/pair_extension_screen.py`: 2023+ PF 5.81, test-2025+ PF 5.94, ddR 1.2, vol-scaled filters — trend_thr 24p, min_buf 1.6p, via `*_by_symbol` config overrides; diversifies off GBP. **EURUSD added 2026-07-24** via the same screen: 2023+ PF 3.69, test-2025+ PF 4.02, N=85, ddR 2.1, real costs (0.7p comm); vol-scaled ×0.47 — trend_thr 14p, min_buf 0.9p; diversifies off JPY entirely. Deployed ahead of the "verify USDJPY live first" gate on the demo-data-collection rationale — the pre-screened, uncorrelated pair is +EV to collect and the only real demo risk is data integrity, not money), pending stop orders, EOD force-close 20:00 UTC. Research mode at 0.5% risk (collecting 30–50 fills). ✅ **Live geometry VALIDATED 2026-07-15** (`scripts/asb_live_geometry_bt.py`): the deployed broker-label geometry (range ≈ true UTC 21:00–04:00, armed 07:00 — the 2026-07-02 audit's "~3h late" finding) was honestly re-backtested on IC M15 with real spread + 1p commission: **N=183 (2022-07→now), 60% WR, PF 1.57; 2023+: PF 1.64; positive every full year (1.30–3.07); ~4 fills/mo**. Sim reproduces live brackets/fills exactly (7/7, 7/8, 7/13 stops to the pip). Slightly below the old costless-wrong-geometry PF 1.79 claim, but real. Caveats: sim fills already-through-at-07:00 days that live skips (retcode 10015, slight sim optimism). **BE12 overlay added 2026-07-15** (`scripts/asb_eod_traintest.py`, train/test split, pre-committed adoption rule): SL moves to entry at 12:00 UTC — the EOD-exit drag (−4.3R/82) came from afternoon faders; BE12 scored train PF 6.58/avgR +0.265 vs baseline 1.55/+0.134 and untouched test PF 3.73/+0.203 vs 1.60/+0.139 with drawdown halved (whole BE-hour family dominated, not one cell). Expect lower WR (scratches book as ~−1.5p) but ~1.5× expectancy. Live so far: 5W/7L, −$131 (all losses = dropped EURJPY; GBPJPY 3W/0L).
  - **LONDON_BREAKOUT (LONDON_BO)** — Asian-range breakout, Mon/Tue only, H1. **Multi-instrument since 2026-07-16: GBPUSD (1% risk, band 10–22p) + GBPJPY (0.5% research size, vol-scaled band 19–42p — added via pre-registered pair screen: 2023+ PF 2.21, test-2025+ PF 2.90; correlation check vs ASB/GBPJPY showed near-complementary range filters, 18 same-day overlaps in 4y, `scripts/gbpjpy_corr_check.py`).** The live geometry (a broker-time accident: 4-bar range ≈ UTC 00–04, traded UTC 08–13) was **re-validated 2026-07-02 as the best of 4 variants** on IC data with commission: N=260, PF 1.63, +929p, dd 108p over 8.3y (2023+: PF 1.62). Keep as deployed — the *documented* "00–07 UTC range" variant is breakeven-to-losing; see `scripts/lbo_geometry_validation.py`. **Range band widened 12–20 → 10–22 pips 2026-07-15** (equal-risk deep analysis, `scripts/lbo_range_sweet_spot.py`): the edge is a broad plateau ~8–25p with no cliff at 20p; 10–22 dominates 12–20 in both full-history (PF 1.76 +81.4R vs 1.63 +50.8R) and 2023+ (PF 1.81 +42.0R vs 1.62 +24.3R) windows. Don't widen past ~22–25p: the 28–35p bucket is PF 0.96 in 2023+. Expect ~3 fills/mo (was ~2). Live: 6W/2L +$131.
...
ASIAN_SESSION_BREAKOUT = {"enabled": True, "instruments": ["GBPJPY","USDJPY","EURUSD"], "risk_pct": 0.5, "eod_force_close_hour": 20, "min_buffer_pips_by_symbol": {"USDJPY": 1.6, "EURUSD": 0.9}, "trend_filter_threshold_pips_by_symbol": {"USDJPY": 24, "EURUSD": 14}}
LONDON_BREAKOUT      = {"enabled": True,  "instruments": ["GBPUSD","GBPJPY"], "days": [0,1], "risk_pct": 1.0, "risk_pct_by_symbol": {"GBPJPY": 0.5}, "range_by_symbol": {"GBPJPY": [19,42]}}
...
- **Parked (screened, passed, not deployed)**: LBO/EURUSD + LBO/EURGBP marginal passes (see `scripts/pair_extension_screen.py`). (ASB/EURUSD was here — deployed 2026-07-24, see ASB in Current State.)
