...
- **Intraday FX breadth ("buy validatability with frequency")** — screened 2026-07-30 (`scripts/fx_breadth_screen.py`, both sanity gates passed: 8/8 incumbent Donchian pins on PF *and* N, plus an engine-identity check proving the second simulator reproduces raw close-to-close moves exactly). The premise was sound and is why it was worth one run: every existing edge is unfalsifiable (ASB needs 923 fills = 26yr; USTEC 5,916yr), and `N ≈ 7.85·sd²/avgR²` says the *only* fix is fill rate. Frequency had to come from **breadth** — 16y of IC H1 (2010-08+), 10 FX pairs, universe chosen by measured friction (round-trip cost ≤ 6% of median H4 ATR20), H4-scale stops so friction is ~0.04R rather than the 0.077R an H1 stop costs on EURUSD. **Both families died, and NOT on cost.**
  - **F1 "H4 Donchian"** (entry_lb ∈ {20,55} H4 bars, exit_lb = entry_lb/2.75, ATR20×1.0, close-based, no SL overlay — deliberately the one family immune to both 2026-07-28 fill fictions): **not one cell was positive even on train** (avgR −0.060 / −0.090). Test 2022+ PF 0.68, FULL 0.84. Decisive column: **gross, zero cost and no financing, is FULL avgR −0.044 / TEST −0.138.** There is no edge to pay costs out of, so no cost assumption can rescue it. A *cleaner* kill than LBO, which at least had 8/960 train-positive cells to overfit.
  - **F2 "unconditional USD session drift"** (enter at close of true-UTC hour ∈ {7,11,15,19}, hold 4 or 8h, one fitted direction per cell pooled in USD terms — 8 df total, not 8/pair): all 16 cells negative on train. Gross FULL avgR **+0.011** (PF 1.04), gross TEST **−0.004**. So unconditional hour-of-day drift on the USD majors is **exactly zero gross**, and net is −0.049 purely because friction is 0.058R — **5× the entire gross effect**. The cost wall measured directly rather than argued. Kills the *unconditional* form only; a conditional/regime variant would need a genuinely new hypothesis, not a re-screen.
  - **The generalizable lesson, and the part worth keeping: high N makes an edge MEASURABLE, it does not make one EXIST — and the friction you spend to buy the N can exceed the edge you were trying to measure.** Trade count multiplies friction linearly while doing nothing for avgR. Before chasing fills to fix a power problem, check that the gross edge exceeds `round_trip / stop` by a comfortable multiple; here it was 0.2× — 5× the wrong way. Do not re-open intraday FX on the frequency argument; it is answered.
  - ⚠️ Methodological defect to fix in any reuse: the friction ceiling floored on **one live spread snapshot**, and the universe changed between two runs 20 min apart (EURJPY 12→19 points, GBPJPY 15→22, NZDUSD 10→15 ⇒ 6 instruments IN became 4). Floor on a *distribution* of snapshots or the recorded-spread upper quantiles, never an instant. Verdicts were unaffected (F1 failed on all 6 and on the 4).
- **Crypto alts on the BTC_DONCHIAN rule** — screened 2026-07-30 (`scripts/crypto_alt_prescreen.py` then `scripts/crypto_alt_screen.py`; incumbent gate 8/8 on PF *and* N). The premise was the strongest available: crypto is the only asset class in the book whose gross edge survives 20%/yr financing, so alts are the same *type* as both survivors. It fails, and it fails for a reason worth keeping.
  - **The candidate set is TWO instruments, and that is a data fact, not a judgement.** IC lists 18 crypto symbols; only 4 have the ~7.3 years needed for N≥40 at ~6 fills/yr — BTCUSD (2011), LTCUSD (2011), ETHUSD (2016), BCHUSD (2017-08) — and two of those are already deployed. Everything else starts 2021-06 (ADAUSD, BNBUSD, DOGUSD, DOTUSD, LNKUSD, UNIUSD, XLMUSD, XTZUSD), 2022-03 (AVXUSD, KSMUSD, POLUSD) or 2025-01 (SOLUSD 11 fills, XRPUSD 6). **This avenue is closed by history until ~2028** — do not revisit in a few months, and do not ask "what about SOL".
  - **BCHUSD FAIL, LTCUSD FAIL, and both fail GROSS** (zero cost, no financing): BCHUSD gross 2017+ PF 1.00 / avgR +0.068, decision column (spread-as-%-of-ATR + financing) 0.86 / −0.068. LTCUSD gross 1.25 FULL but **held-out 2022+ gross PF 0.25 / avgR −0.660** — it stopped trending after 2021; decision column 0.94 FULL / 0.16 test. No cost assumption is load-bearing.
  - **The finding that generalizes: "crypto trends" is NOT the edge — BTC/ETH specifically are.** Same rule, same era, same class: gross avgR is BTCUSD **+4.67R** and ETHUSD **+6.98R** versus BCHUSD **+0.068R** and LTCUSD **+0.32R**, i.e. a ~20–100× gap, with WR 37%/33% vs 9%/13%. The alts break out and fail to follow through far enough relative to their own ATR to pay for a 90% loss rate. Do not reason "it worked on BTC because crypto trends" — that inference is now measured false, which also means the incumbents' edge is narrower and more fragile than a class-level story implies.
  - ⚠️ **Two of my own priors were wrong and are corrected in the script's docstring.** (1) I asserted alts correlate 0.7–0.9 with BTC so this could only buy capacity. **Measured monthly-R correlation is BCHUSD/BTCUSD −0.01, LTCUSD/BTCUSD +0.08** — strategy returns decorrelate even when prices don't, because R depends on each instrument's own trigger timing and follow-through vs its own ATR (the index/gold additions came in at mean 0.07 too). **Never pre-reject an extension in this family on price correlation; measure monthly R.** (2) The pre-screen's friction ceiling (5% of ATR20) was mis-calibrated ~6× too tight for a family with +4.6R gross edge, and rejected both candidates before scoring; it was deliberately overridden in the permissive direction with the deciding bar left untouched. A gate calibrated against friction alone is wrong when the edge is huge — calibrate it as a fraction of *expected edge*.
  - **New cost-model tool, reusable: `cost_mode "atrfrac"`.** LTCUSD's recorded spread field is 0 across its entire 15-year history, so the incumbents' floor-at-a-live-snapshot trick degenerates to charging today's *dollars* on a coin that traded at $2 and at $300 — the identical fixed-amount-vs-rate error that inflated USTEC's financing 5×. Holding spread/ATR constant instead fixes it. Note the correction is **not** uniformly conservative: a fixed amount over-charges low-vol eras and under-charges high-vol ones (LTCUSD 0.89→0.94, BTCUSD 4.48→4.55). Both candidates still fail on the *more lenient* column.
