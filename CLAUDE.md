...
- **Active strategies** (each its own scanner thread; `ENABLED_PATTERNS` for the main loop is now `[]`):
  ...
  - **ASIAN_SESSION_BREAKOUT (ASB)** — Asian-range breakout on **GBPJPY only** (range ≈ true UTC 21:00–04:00, armed 07:00), pending stop orders, EOD force-close 20:00 UTC, research mode at 0.5% risk. ⚠️ **Audited 2026-07-28 (`scripts/asb_fill_audit.py`) — every previously recorded PF for this strategy was inflated by two sim fictions.** Read the audit before trusting any older number here.
    ...
    - ⚠️ **GBPJPY fails the honest screen's own time-stability legs** (2026-07-29): 2023+ PF 1.28 clears the 1.25 bar, but per-year avgR is +0.18 / +0.10 / **−0.06** (2023/24/25) and the 2025+ test leg is PF 1.00, avgR −0.001 over N=63. The edge is concentrated in 2023–24 and is flat since. Not acted on — ASB is research-mode at 0.5% and under the "no param changes until 30–50 clean live fills" rule — but this is the thing to watch, and it means live GBPJPY is running on an edge the honest data no longer shows in the most recent year.
