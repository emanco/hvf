...
- `get_open_positions()` None-vs-empty hardening (order_manager.py:471): `mt5.positions_get()` returns `None` on IPC error but `()` when genuinely flat, and `if not positions: return []` collapses both. A `None` (error) masquerading as "no positions" could make reconciliation panic-close live trades. **Latent/unproven** — already guarded by the `is_mt5_healthy()` (`account_info()`) gate before recon's missing-detection, and no evidence it has ever fired (the 2026-07-27 trade-232 "false close" turned out to be a units error: recon acted 2m25s *after* the genuine close; the real defect was the deal-lookup skew, since fixed). Clean fix if ever wanted: return `None` from `get_open_positions()` on `positions_get() is None` and have reconciliation skip the cycle (as it does for the health gate). One caller (reconciliation.py:109).
